...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        pub_sheet = st.selectbox("Publications sheet", options=["None"] + sheets, index=(["None"] + sheets).index("Publications") if "Publications" in sheets else (["None"] + sheets).index("Papers") if "Papers" in sheets else 1)
        grant_sheet = st.selectbox("Grants sheet", options=["None"] + sheets, index=(["None"] + sheets).index("Grants") if "Grants" in sheets else 0)
        trial_sheet = st.selectbox("Clinical Trials sheet", options=["None"] + sheets, index=(["None"] + sheets).index("Clinical Trials") if "Clinical Trials" in sheets else (["None"] + sheets).index("Trials") if "Trials" in sheets else 0)
