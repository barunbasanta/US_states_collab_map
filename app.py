...
def count_sheet(
    ...
    # Count every parsed state, including Massachusetts.
    # This keeps Massachusetts P/G/T/Total in the map data and download CSV.
...
def color_scale_for_mode(mode: str):
    # Zero-count states are white in every gradient.
    # Counts from 1 onward use the selected gradient.
    gradients = {
        "Red → Yellow": [
            [0.00, "#FFFFFF"],  # 0 only
            [0.01, "#FFFFFF"],
            [0.02, "#FFF7D6"],  # 1–2 lightest yellow
            [0.25, "#FFF7D6"],
            [0.26, "#FFD07A"],
            [0.50, "#FFD07A"],
            [0.51, "#F89C3D"],
        ...
        "Blue → Cyan": [
            [0.00, "#FFFFFF"],  # 0 only
            [0.01, "#FFFFFF"],
            [0.02, "#E6F7FF"],  # 1–2 lightest blue/cyan
            [0.25, "#E6F7FF"],
            [0.26, "#CFEFFF"],
            [0.50, "#7FD3FF"],
            [0.51, "#2B8CFF"],
        ...
        "Green → Mint": [
            [0.00, "#FFFFFF"],  # 0 only
            [0.01, "#FFFFFF"],
            [0.02, "#EAF9EF"],  # 1–2 lightest green
            [0.25, "#EAF9EF"],
            [0.26, "#DDF8E8"],
            [0.50, "#98E2B8"],
            [0.51, "#43B97F"],
        ...
        "Purple → Pink": [
            [0.00, "#FFFFFF"],  # 0 only
            [0.01, "#FFFFFF"],
            [0.02, "#F8EAFE"],  # 1–2 lightest purple/pink
            [0.25, "#F8EAFE"],
            [0.26, "#F3D9FF"],
            [0.50, "#D8A8FF"],
            [0.51, "#B05CFF"],
    ...
    return gradients.get(mode, gradients["Red → Yellow"])


def category_value(total: int, mode: str) -> int:
    # Separate zero from low nonzero counts.
    if total == 0:
        return 0
    if total <= 2:
        return 1
...
def make_map(
    ...
    fig.add_trace(
        go.Choropleth(
            ...
            colorbar=dict(
                title=dict(
                    text="<b>Total<br>Collaborations</b>",
                    side="top",
                    font=dict(size=18, color="black"),
                ),
                tickmode="array",
                tickvals=tickvals,
                ticktext=[f"<b>{t}</b>" for t in ticktext],
    ...
    # Positioned in the bottom-right open space, just left of the color scale,
    # so it does not overlap Florida.
    fig.add_annotation(
        x=0.975,
        y=0.195,
        xref="paper",
    ...
    # Northeast compact table for small states that cannot be labeled clearly on the map.
    # Format: State abbreviation + Total, then P / G / T on the next line.
    ne_table_order = ["NH", "VT", "MA", "RI", "CT", "NJ", "DE", "MD"]
    ne_rows = []
    for abbr in ne_table_order:
        match = counts_df[counts_df["Abbr"] == abbr]
        if match.empty:
            continue
        row = match.iloc[0]
        ne_rows.append(
            f"<b>{abbr} {int(row['Total'])}</b><br>"
            f"<span style='color:#0057E7'><b>{int(row['P'])}</b></span>"
            f" / "
            f"<span style='color:#14833B'><b>{int(row['G'])}</b></span>"
            f" / "
            f"<span style='color:#8B4513'><b>{int(row['T'])}</b></span>"
        )
    ...
    if title:
        fig.update_layout(
            title=dict(text=title, x=0.5, xanchor="center", font=dict(size=30, family="Arial Black")),
        )
    ...
    fig.update_layout(
        ...
        paper_bgcolor="white",
        plot_bgcolor="white",
        margin=dict(l=10, r=80, t=70 if title else 15, b=10),
...
st.caption("Upload an Excel workbook and export a high-quality PNG, PDF, or SVG map. Massachusetts is counted like every other state.")
...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        primary_state = st.text_input("Primary state/reference state", value="Massachusetts")
...
if uploaded_file:
    ...
    with right:
        ...
        try:
            ...
            st.error(
                "Export failed. On Streamlit Cloud, use `kaleido==0.2.1` in requirements.txt "
                "so Chrome is not required for PNG/PDF/SVG export."
            )
            st.exception(e)
